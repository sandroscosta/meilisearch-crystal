# The `meilisearch-crystal` shard: a type-safe Meilisearch client for Crystal.
#
# ```
# require "meilisearch-crystal"
# ```
require "./meilisearch/crystal/version"
require "./meilisearch/crystal/error"
require "./meilisearch/crystal/exceptions"
require "./meilisearch/crystal/resource"
require "./meilisearch/crystal/task"
require "./meilisearch/crystal/api"
require "./meilisearch/crystal/config"
require "./meilisearch/crystal/client"
require "./meilisearch/crystal/tasks"
require "./meilisearch/crystal/index"
require "./meilisearch/crystal/indexes"
require "./meilisearch/crystal/documents"
require "./meilisearch/crystal/index_client"

module Meilisearch::Crystal
end
